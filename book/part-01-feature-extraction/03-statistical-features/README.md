...
特征提取是时间序列分析的首要步骤。好的特征能够以少量标量或向量刻画序列的核心模式，为后续的分类、异常检测、预测与因果分析提供可解释且稳定的输入 [^christ2018tsfresh]。本章从统计、时域、频域三个视角梳理常用特征：统计特征回答“序列取值的分布如何”，平稳性、季节性、趋势性与周期性等时域特征回答“序列随时间怎样演化”，频域特征则回答“序列的能量分布在哪些周期上”。
...
本章所有示例均可在 Python 3.9+ 环境中运行，主要依赖 `numpy`、`pandas`、`scipy`、`statsmodels` 与 `matplotlib` [^mckinney2011pandas]。`assets/code/` 目录下提供了 7 个独立脚本，分别对应本章 3.1–3.6 节内容。建议先安装依赖，再逐个运行脚本并观察输出。
...
<table>
  ...
  <tbody>
    <tr>
      <td rowspan="5"><strong>统计特征</strong></td>
      <td>均值、中位数、众数</td>
      <td>描述序列取值的中心位置</td>
    </tr>
    <tr>
      <td>方差、标准差、极差、四分位距（IQR）</td>
      <td>描述序列取值的离散程度与分布范围</td>
    </tr>
    <tr>
      <td>偏度、峰度</td>
      <td>刻画分布的不对称性与尾部厚度</td>
    </tr>
    <tr>
      <td>绝对能量、均方根（RMS）、过零率（ZCR）</td>
      <td>刻画序列整体强度与穿越参考水平的频繁程度</td>
    </tr>
    <tr>
      <td>滚动窗口统计量</td>
      <td>在滑动窗口内重复计算统计量，捕捉序列的局部时变特性</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>平稳性特征</strong></td>
      <td>ADF 统计量与 p-value</td>
      <td>单位根检验：原假设为序列存在单位根（非平稳）</td>
    </tr>
    <tr>
      <td>KPSS 统计量与 p-value</td>
      <td>平稳性检验：原假设为序列平稳</td>
    </tr>
    <tr>
      <td>滚动均值/方差变异系数</td>
      <td>局部窗口内统计量的波动程度，衡量局部平稳性</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>季节性特征</strong></td>
      <td>季节性强度 $F_s$</td>
      <td>基于 STL 分解，衡量季节项解释的方差比例</td>
    </tr>
    <tr>
      <td>周期内统计量</td>
      <td>按固定周期位置分组，计算每个相位上的均值、标准差等</td>
    </tr>
    <tr>
      <td>季节滞后自相关（ACF）</td>
      <td>在周期倍数滞后处的自相关系数，反映季节性重复强度</td>
    </tr>
    <tr>
      <td rowspan="4"><strong>趋势性特征</strong></td>
      <td>线性趋势斜率、截距、$R^2$</td>
      <td>线性回归拟合序列长期演化方向</td>
    </tr>
    <tr>
      <td>趋势强度 $F_t$</td>
      <td>基于 STL 分解，衡量趋势项解释的方差比例</td>
    </tr>
    <tr>
      <td>Mann-Kendall $S$ 统计量</td>
      <td>非参数 Kendall's $S$，衡量单调趋势的显著性</td>
    </tr>
    <tr>
      <td>滚动斜率符号变化</td>
      <td>局部窗口线性斜率的方向转折次数</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>周期性特征</strong></td>
      <td>ACF 峰值周期</td>
      <td>自相关函数越过置信界的第一个显著峰值对应滞后</td>
    </tr>
    <tr>
      <td>周期图峰值周期</td>
      <td>FFT 功率谱主导频率对应的周期</td>
    </tr>
    <tr>
      <td>滚动周期稳定性</td>
      <td>滚动窗口周期估计的标准差，衡量周期是否稳定</td>
    </tr>
    <tr>
      <td rowspan="3"><strong>频域特征</strong></td>
      <td>主导频率、主导幅值、低/高频能量比</td>
      <td>FFT 将序列分解为不同频率分量，从能量角度揭示周期性</td>
    </tr>
    <tr>
      <td>谱熵、谱质心、谱带宽</td>
      <td>Welch PSD 描述功率在频率上的分布形态</td>
    </tr>
    <tr>
      <td>频带能量占比</td>
      <td>将频谱划分为低/中/高频带，计算各带能量占总能量之比</td>
    </tr>
  </tbody>
</table>
...
**平稳性（Stationarity）** 是指时间序列的统计特性不随时间推移而显著变化。严格来说，可分为两类：

- **严平稳（Strict Stationarity）**：序列任意有限维分布都不随时间平移而变化。
- **宽平稳（Weak / Covariance Stationarity）**：序列的均值、方差为常数，且自协方差仅依赖于滞后阶数而非绝对时刻。

工程上通常讨论的是宽平稳。若序列的均值、方差或自相关结构随时间漂移，则称其为**非平稳**。平稳性是许多经典时序模型（如 ARMA、ARIMA）的重要前提：非平稳序列的均值或方差漂移会让参数估计失去意义，而平稳序列更易于建模与预测。

在实际工程中，完全没有趋势或季节性的序列很少见。我们更常关注的是：序列在去除确定性趋势与季节性后，残差是否平稳；或者序列的局部窗口内是否近似平稳。
...
**季节性（Seasonality）** 是指时间序列在固定时间间隔（如小时、天、周、年）上重复出现的模式。与趋势性不同，季节性通常围绕一个相对稳定的周期波动，例如电力负荷的日内峰谷、零售销量的周末效应、交通流量的早晚高峰等。

季节性有两个关键特点：

1. **周期固定且通常与日历相关**：如 24（小时）、7（天）、12（月）、4（季度）。
2. **波形在周期内相对稳定**：每个周期相同相位上的取值分布相似。

刻画季节性不仅有助于理解业务规律，还能为后续的去季节、预测与异常检测提供结构化特征。需要注意，季节性强调的是“固定日历间隔”，而周期性（见 3.5 节）可以发生在任意长度的时间间隔上，不一定与日历相关。
...
**趋势性（Trend）** 是指时间序列在较长时间尺度上呈现的持续上升、下降或保持稳定的倾向。与季节性不同，趋势通常不呈现固定周期，而是反映序列整体的演化方向，例如业务增长、设备老化、温度长期变化等。

趋势可以是：

- **确定性趋势**：由时间或其他外生变量决定，如线性趋势、多项式趋势。
- **随机趋势**：由随机游走或单位根过程产生，难以用简单函数拟合。

准确地刻画趋势不仅有助于理解序列的长期行为，也能为去趋势、预测与异常检测提供关键特征。在工程实践中，趋势性常与季节性同时存在，通常先用 STL 等方法将两者分离，再分别提取特征。
...
**周期性（Periodicity）** 是指序列中重复出现的波动模式，其周期长度可以与日历无关。与季节性强调固定日历间隔（如小时、天、周）不同，周期性更关注数据本身内在的循环长度，例如机械振动、心电节律或网络流量的 burst 周期。

周期性的关键特点是：

1. **周期长度由数据本身决定**，不一定与日历对齐。
2. **周期可能不稳定**，会随时间缓慢变化。

刻画周期性不仅需要估计周期长度，还需要评估周期是否稳定。工程中常用 ACF 峰值法、周期图峰值法以及滚动窗口周期稳定性来综合判断。
...
本章从统计、时域、频域三个层面系统梳理了时间序列特征提取方法：

- **统计特征**（3.1 节）刻画序列取值的分布与局部窗口特性，包括基本统计量、偏度峰度、能量与过零率、滚动窗口统计量等。它们是特征工程的基础，计算简单、可解释性强。
- **平稳性特征**（3.2 节）刻画序列的统计特性是否随时间漂移。ADF 与 KPSS 检验从不同假设出发给出全局判断，滚动统计量变异系数则可衡量局部平稳性。
- **季节性特征**（3.3 节）刻画固定日历间隔上的重复模式。通过 STL 分解、季节性强度、周期内统计量与季节滞后 ACF，可以从波形形态与相位两个角度量化季节性。
- **趋势性特征**（3.4 节）刻画序列长期的上升、下降或稳定倾向。线性趋势、趋势强度、Mann-Kendall 检验与滚动斜率符号变化提供了从全局到局部的多层次趋势描述。
- **周期性特征**（3.5 节）刻画数据内在重复波动的周期长度与稳定性。ACF 峰值法、周期图峰值法与滚动周期标准差相互补充，可避免单一方法在噪声或变周期场景下的误判。
- **频域特征**（3.6 节）从能量分布角度揭示周期性、振动与噪声。FFT 主导频率、Welch PSD、谱熵、谱质心、谱带宽与频带能量占比等指标，在振动、声学、网络流量等场景中尤为有效。

在实际工程中，通常需要将多类特征按固定顺序拼接为特征向量，并根据任务需求（分类、异常检测、预测）选择合适的子集与模型。多尺度窗口、特征标准化、缺失值处理与特征选择是后续章节将重点讨论的工程实践。
