...
特征提取是时间序列分析的首要步骤。好的特征能够以少量标量或向量刻画序列的核心模式，为后续的分类、异常检测、预测与因果分析提供可解释且稳定的输入 [^christ2018tsfresh]。本章从统计特征、时域特征与频域特征三个层面展开，并讨论特征组合与工程实践中的常见问题。
...
- 能够使用 Python（`numpy`、`pandas`、`scipy`、`statsmodels`）计算并可视化上述特征 [^mckinney2011pandas]。

## 1. 引言

时间序列的原始形式通常是一条按时间排列的数值序列 $x_1, x_2, \dots, x_N$。直接将这些原始点输入模型会面临维度高、噪声大、模式不鲜明等问题。特征提取的核心任务，就是把这条序列转化为少量、稳定、可解释的标量或向量，从而暴露序列在统计分布、时间结构、周期能量等方面的规律。

一个好的时序特征应当具备三个性质：

1. **可区分性**：不同类别或状态下的序列在特征空间中能够分开。
2. **稳健性**：对采样噪声、异常点、长度变化不敏感。
3. **可解释性**：特征值本身能够对应到业务或物理含义，便于调试与信任。

本章围绕“统计—时域—频域”三条主线组织。统计特征回答“序列取值的分布如何”；时域特征回答“序列随时间怎样演化”；频域特征回答“序列的能量分布在哪些周期上”。最后，我们将这些特征组合成向量，并讨论多尺度窗口与任务适配的工程实践。

## 2. 核心概念

特征提取不是孤立的步骤，而是连接原始数据与下游模型的桥梁。下面先明确几个贯穿本章的核心概念。

**特征与特征向量**  
单个特征 $f$ 是从序列 $\mathbf{x}$ 到标量的映射；多个特征按固定顺序拼接后形成特征向量 $\mathbf{f} = [f_1, f_2, \dots, f_D]^\top$。对于 $M$ 条序列，可得到特征矩阵 $\mathbf{F} \in \mathbb{R}^{M \times D}$。

**全局特征与局部特征**  
全局特征（如均值、方差）刻画整条序列；局部特征（如滚动窗口统计量）刻画某段时间窗口内的行为。二者互补：全局特征稳定，局部特征能捕捉动态变化。

**时间结构与频谱结构**  
时域特征关注序列在不同时刻的相关性、趋势与周期；频域特征则通过傅里叶变换把序列分解为不同频率分量，从能量角度揭示周期性。

**多尺度分析**  
同一特征在不同窗口长度下会呈现不同信息。短窗口对突发扰动敏感，长窗口反映长期水平。工程上通常同时提取多个尺度，再聚合为固定维度。
...
**从低阶矩到高阶形态**  
先提取均值、方差等一、二阶矩，再补充偏度、峰度等分布形态特征，可逐步细化对序列数值行为的刻画。

**从全局到局部、从静态到动态**  
全局统计量给出整体画像；滚动窗口统计量、滚动 ADF/KPSS、滚动周期估计等局部特征能够刻画序列的时变特性。

**时域与频域互补**  
时域特征直观、可解释；频域特征对周期性、振动、噪声更敏感。二者结合往往能取得比单一视角更好的效果。

**特征组合与尺度聚合**  
把多类特征拼接后，通常需要进行标准化、缺失值处理、特征选择。多尺度窗口的统计量可通过均值、标准差、最大/最小值聚合，避免维度爆炸。

**可复现性**  
特征提取应记录采样频率、窗口长度、滞后阶数、去趋势方式等超参数，并使用固定随机种子，确保实验可复现。

## 4. 算法步骤

给定一条时间序列 $\mathbf{x} = [x_1, \dots, x_N]$，构建特征向量可按以下步骤进行：

1. **数据预处理**：检查缺失值、异常值与采样一致性；必要时进行重采样、去趋势或去季节。
2. **提取全局统计特征**：计算均值、中位数、标准差、偏度、峰度、极差、IQR、绝对能量、RMS、过零率等。
3. **提取局部窗口特征**：选取一组窗口长度 $W = \{w_1, \dots, w_K\}$，在每个窗口内计算均值、标准差、极差、偏度、峰度，并聚合为各窗口统计量的均值、标准差、最大/最小值。
4. **提取时域结构特征**：
   - 平稳性：ADF 统计量与 p-value、KPSS 统计量与 p-value、滚动均值/方差变异系数。
   - 季节性：STL 分解、季节性强度、周期内统计量、季节滞后 ACF。
   - 趋势性：线性趋势斜率与 $R^2$、趋势强度、Mann-Kendall $S$、滚动斜率符号变化次数。
   - 周期性：ACF 峰值周期、周期图峰值周期、滚动周期标准差。
5. **提取频域特征**：FFT 主导频率与幅值、低/高频能量比、Welch PSD 谱熵/谱质心/谱带宽、频带能量占比。
6. **特征组合与清洗**：将所有标量按固定顺序拼接，处理缺失值，必要时进行标准化或对数变换。
7. **下游建模**：根据任务选择分类、检测或回归模型，并记录特征提取超参数。

实际实现时，不必每一步都使用；应根据数据特点与任务需求选择子集。

## 5. Python 实践

本章所有示例均可在 Python 3.9+ 环境中运行，主要依赖 `numpy`、`pandas`、`scipy`、`statsmodels` 与 `matplotlib`。`assets/code/` 目录下提供了 8 个独立脚本，分别对应：

- `03-example-01-basic-stats.py`：基本统计量与分布形态
- `03-example-02-rolling-stats.py`：滑动窗口统计量
- `03-example-03-stationarity-features.py`：平稳性特征
- `03-example-04-seasonality-features.py`：季节性特征
- `03-example-05-trend-features.py`：趋势性特征
- `03-example-06-periodicity-features.py`：周期性特征
- `03-example-07-frequency-features.py`：频域特征
- `requirements.txt`：本章特殊依赖

建议按以下流程实践：

1. 安装依赖：`pip install -r assets/code/requirements.txt`（通用依赖 `numpy`、`pandas`、`scipy`、`statsmodels`、`matplotlib`、`scikit-learn`、`lightgbm` 需自行安装）。
2. 逐个运行脚本，观察输出图片与数值。
3. 替换脚本中的合成数据为你自己的时间序列，验证特征是否稳定。
4. 将多类特征拼接成向量，尝试简单的分类或异常检测任务。

下面各节将详细展开每类特征的原理、公式与代码实现。
...
其中 $\Delta x_t = x_t - x_{t-1}$ 为一阶差分，$p$ 为滞后阶数。原假设为序列存在单位根（非平稳）；当 p-value 小于显著性水平（通常取 0.05）时，拒绝原假设，认为序列平稳 [^dickey1979adf]。
...
**KPSS（Kwiatkowski-Phillips-Schmidt-Shin）检验** 与 ADF 检验的假设正好相反：原假设为序列平稳，备择假设为序列存在单位根 [^kwiatkowski1992kpss]。因此解读时需要注意 p-value 的方向：
...
给定序列的 STL 分解 $x_t = T_t + S_t + R_t$，其中 $T_t$ 为趋势项、$S_t$ 为季节项、$R_t$ 为残差项 [^cleveland1990stl]，**季节性强度（Seasonal Strength）** 可定义为：
...
#### 7.3.3 Mann-Kendall 趋势检验

**Mann-Kendall 检验** 是一种非参数趋势检验 [^mann1945trend]，不假设数据分布，对异常值较为稳健。其核心统计量为 Kendall's $S$：
...
在时间序列预测中 [^hyndman2018forecasting]，特征向量可作为回归模型的输入，用于预测未来一个或多个时刻的值。例如，在电力负荷预测中，可提取日内周期性相位统计量、趋势斜率、历史同期均值与频带能量比，作为 **XGBoost** 或 **LightGBM** 的特征，辅助模型捕捉周期与趋势。
...
本章生成的 7 张图片分别对应不同特征的直观解释。

- **图 3-1 基本统计量概览**：展示序列的均值、中位数、标准差、四分位数等全局指标，帮助快速判断中心位置与离散程度。
- **图 3-2 滑动窗口统计量概览**：通过滚动均值与滚动标准差，观察局部平稳性与波动聚集区。
- **图 3-3 平稳性特征示例**：对比平稳序列与非平稳序列的 ADF/KPSS 检验结果与滚动统计量，理解两种检验假设相反的特点。
- **图 3-4 STL 分解与季节性强度**：将序列分解为趋势、季节与残差，季节性强度越接近 1 说明季节性越显著。
- **图 3-5 趋势性特征示例**：同时展示线性拟合、STL 趋势、Mann-Kendall 统计量与滚动斜率符号变化，综合评估趋势。
- **图 3-6 周期性特征示例**：ACF 峰值与周期图峰值给出周期估计，滚动窗口周期标准差反映周期稳定性。
- **图 3-7 频域特征示例**：FFT 幅频谱、Welch 功率谱与频带能量占比，揭示主导频率与能量分布。

解读时应注意：单张图或单个特征往往不足以下结论，应结合多个视角。例如，ADF 与 KPSS 结论冲突时，可查看滚动统计量；周期图出现多个峰值时，可结合 ACF 判断真实周期。
...
**1. 采样频率必须一致**  
大部分特征（如 FFT 频率、周期估计、滚动窗口）都隐含假设序列采样频率固定。如果数据存在不规则采样，应先进行重采样或插值。

**2. 窗口长度的选择**  
窗口太短会放大噪声，窗口太长会抹平局部变化。建议结合业务周期（如 24、168、365）与网格搜索选择窗口长度。

**3. 平稳性检验结论可能冲突**  
ADF 的原假设是“存在单位根”，KPSS 的原假设是“平稳”。当两者冲突时，通常说明序列是结构变化、趋势非线性或接近边界平稳，应结合滚动统计量与可视化判断。

**4. 分布形态特征对异常值敏感**  
偏度、峰度等特征受极端值影响较大。若序列中存在显著异常点，建议先进行稳健估计或分位数统计。

**5. 频域特征需要足够长度**  
FFT 与 Welch 法的频率分辨率分别为 $f_s/N$ 与 $f_s/n_{\text{perseg}}$。序列过短会导致频率混叠或周期估计不准。

**6. 特征维度爆炸**  
多尺度窗口会快速增加特征维度。建议先进行特征重要性分析或相关性筛选，避免过拟合。

**7. 可复现性**  
记录采样频率、窗口长度、滞后阶数、去趋势/去季节方式等超参数，并使用固定随机种子，确保实验可复现。
