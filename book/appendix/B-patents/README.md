...
本附录收录北京必示科技有限公司公开的智能运维相关专利，作为正文异常检测、根因分析、时序预测等章节的工业实践参考。
...
|---|---|---|
| `pdfs/` | 专利原始 PDF | 30 件 |
| `docs/` | 每件专利的中文方案说明 | 30 件 |

## 维护方式

索引页由 `tools/build_index.py` 根据 `patents_index.csv` 与 `docs/` 下的 README 自动生成。更新后请运行：

```bash
python3 tools/build_index.py pdfs
```
